...
if st.button("🚀 Analyse & Export starten"):
    results = []
    ...
    if results:
        df = pd.DataFrame(results).sort_values(by=["Status", "Ü1.5 %"], ascending=[False, False])
        st.dataframe(df, use_container_width=True, hide_index=True)
        
        # CSV Download Buttons
        csv = df.to_csv(index=False).encode('utf-8')
        st.download_button("📥 Alle Ergebnisse (CSV)", data=csv, file_name=f"picks_{date_str}.csv", mime='text/csv')
        
        star_df = df[df["Status"] == "⭐ STAR PICK"]
        if not star_df.empty:
            st.download_button("🌟 Nur STAR PICKS (CSV)", data=star_df.to_csv(index=False).encode('utf-8'), file_name=f"stars_{date_str}.csv", mime='text/csv')
    else:
